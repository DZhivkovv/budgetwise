 ...
 */
export default async function getUserBudget(userId)
{
    try
    {
        ...
        if (userId === undefined || userId === null || isNaN(userId) || Number(userId) <= 0)
        {
            return { success: false, status: 400, message: "Invalid or missing user ID.", data: {} };
        }
        ...
        if (!budget){ 
            return { success:false, status:404, message: "No budget found.", data: {} };
        }

        // Return the user's budget.
        return { success: true, status: 200, message: "User budget retrieved successfully!", data: budget }
    }
    catch (error)
    {
